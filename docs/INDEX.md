# 📚 StudyHub Documentation Index

Welcome to the StudyHub documentation. This directory is organized into logical sections to help you find what you need.

## 📂 Navigation

### 🏗️ Project Overview
- [Project Summary](project/PROJECT_SUMMARY.md) - High-level overview of the platform.
- [Project Status](project/PROJECT_STATUS.md) - Current development roadmap and status.
- [Memory Map](project/PROJECT_MEMORY_MAP.md) - Conceptual architecture and structural overview.
- [Future Roadmap](project/FUTURE_ROADMAP.md) - Planned features and updates.

### 🚀 Getting Started
- [General Setup](setup/SETUP.md) - How to get the project running locally.
- [MySQL Setup](db/MYSQL_SETUP.md) - Database-specific installation and configuration.
- [Environment Variables](setup/UPDATE_ENV.md) - Configuration guide for `.env`.
- [GitHub Setup](setup/GITHUB_SETUP.md) - Repository and deployment setup.

### 🌐 API & Database
- [API Documentation](api/API_DOCUMENTATION.md) - Endpoint reference and usage.
- [Database Schema](db/MYSQL_CHECKLIST.md) - Schema highlights and table details.
- [Migration Guide](db/MYSQL_MIGRATION_SUMMARY.md) - History of database transitions.

### 🚢 Deployment
- [Deployment Guide](deploy/DEPLOYMENT.md) - General deployment instructions.
- [Railway Deployment](deploy/DEPLOY_RAILWAY.md) - Specific guide for Railway.app.
- [Free Hosting Guide](deploy/FREE_HOSTING_GUIDE.md) - Options for hosting without cost.

### 📜 Community & Legal
- [Contributing](CONTRIBUTING.md) - Guidelines for contributing to StudyHub.
- [Security](SECURITY.md) - Security policy and reporting.
- [Branding Guidelines](branding/BRAND_GUIDELINES.html) - Visual identity and design assets.
- [License](../LICENSE) - Project license information.

### 🕒 History
- [Phase 5 Complete](history/PHASE5_COMPLETE.md)
- [Phase 3 Complete](history/PHASE3_COMPLETE.md)
- [Phase 2 Complete](history/PHASE2_COMPLETE.md)
- [Changelog](project/CHANGELOG.md)
- [Release Notes](project/RELEASE_NOTES.md)

---
*For quick reference, see the [Quick Reference Guide](project/QUICK_REFERENCE.md).*
